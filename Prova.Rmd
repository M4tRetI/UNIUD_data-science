...
ggplot(temp2.bis, aes(x = Height, y = prob*100)) +
  ...
  geom_segment(aes(x = Height, xend = Height, y = 0, yend = prob*100)) +
  geom_hline(aes(yintercept=mean(prob*100)), color = "navyblue") +
  geom_text(aes(150,mean(prob*100),label = paste("Media:", mean(prob*100)), vjust = -1), color = "navyblue") +
  geom_segment(aes(x = 169, xend = 169, y = 0.24, yend = 0.825), color = "blue",
               arrow = arrow(length = unit(0.02, "npc"), ends = "both")) +
  geom_text(aes(166,0.6,label = "3.4x"), color = "blue", size = 7) +
  geom_segment(aes(x = 168, xend = 177, y = 0.825, yend = 0.825), color = "blue") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
...
athletes %>%
  ...
  group_by(Year, City) %>%
  summarize(n_ath = n()) %>%
  ggplot(aes(x = Year, y = n_ath, fill = factor(ifelse(Year==1960,"Highlighted","Normal")))) +
  geom_bar(position = "stack", stat = "identity") +
  theme_minimal() +
  theme(legend.position = "none")
# Si più atleti delle vicine, ma nemmeno tanto. Helsinki due edizioni prima e Monaco 3 anni dopo
